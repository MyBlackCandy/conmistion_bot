...
async def set_tz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cur.execute("""INSERT INTO group_permissions (chat_id, user_id, role, tz_offset) 
                       VALUES (%s, %s, 'master', %s) 
                       ON CONFLICT (chat_id, user_id) DO UPDATE SET tz_offset=%s""", 
                    (chat_id, update.effective_user.id, offset, offset))
        conn.commit(); conn.close()
        await update.message.reply_text(f"✅ 本群时区已更新为: GMT {offset:+}")
    except: await update.message.reply_text("💡 格式: `/set_tz [小时]`")


# ฟังก์ชันช่วยแปลงเลขสายงานเป็นภาษาจีน
def get_line_name(n):
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        for d in r['details']:
            ...
            # เปลี่ยนตรงนี้: ใช้ get_line_name
            l_cn = get_line_name(l_no)
...
async def handle_plus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i in range(0, len(p[3:]), 2):
            ...
            # เปลี่ยนตรงนี้: แสดงเป็นภาษาจีนทันที
